...
#pragma omp parallel for if (ncandidates.cols() >= 100)
    ...
    dVec pheno(const dVec &candidate) const
    {
      ...
      if (_id)
        ...
        _boundstrategy.to_f_representation(ncandidate, phen);
  ...
  template <>
  inline dMat GenoPheno<NoBoundStrategy, NoScalingStrategy>::pheno(const dMat &candidates) const
  {
    if (_id)
      return candidates;
    else
      return pheno_candidates(candidates);
  }
  template <>
  inline dVec GenoPheno<NoBoundStrategy, NoScalingStrategy>::pheno(const dVec &candidate) const
  {
    if (_id)
      return candidate;
    else
  ...
  template <>
  inline dVec GenoPheno<NoBoundStrategy, NoScalingStrategy>::geno(const dVec &candidate) const
  {
    if (_id)
      return candidate;
    else
  ...
  template <>
  inline dVec GenoPheno<NoBoundStrategy, linScalingStrategy>::pheno(const dVec &candidate) const
  {
    dVec ncandidate(candidate.rows());
    if (!_id)
      _phenof(candidate.data(), ncandidate.data(), candidate.rows());
    ...
    if (!_id)
      ...
      _scalingstrategy.scale_to_f(candidate, sphen);
    return sphen;
  }
  template <>
  inline dVec GenoPheno<NoBoundStrategy, linScalingStrategy>::geno(const dVec &candidate) const
  {
  ...
  template <>
  inline dMat GenoPheno<NoBoundStrategy, linScalingStrategy>::pheno(const dMat &candidates) const
  {
    dMat ncandidates;
    if (!_id)
...
#pragma omp parallel for if (ncandidates.cols() >= 100)
  ...
  template <>
  inline GenoPheno<NoBoundStrategy, linScalingStrategy>::GenoPheno(const dVec &scaling,
                                                                   const dVec &shift,
  ...
  template <>
  inline GenoPheno<pwqBoundStrategy, linScalingStrategy>::GenoPheno(const dVec &scaling,
                                                                    const dVec &shift,
                                                                    const double *lbounds,
                                                                    const double *ubounds)
      : _id(true)
