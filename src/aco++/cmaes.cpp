...
void cmaes_UpdateEigensystem(cmaes_t *t, int flgforce)
{
  ...
  if (flgforce == 0)
  {
    ...
    if (t->sp.updateCmode.maxtime < 1.00 && t->eigenTimings.tictoctime > t->sp.updateCmode.maxtime * t->eigenTimings.totaltime && t->eigenTimings.tictoctime > 0.0002)
      return;
  }
...
{
  if (filename && (strcmp(filename, "no") == 0 || strcmp(filename, "non") == 0 || strcmp(filename, "none") == 0))
    return 1;
