 ...
 */
{
    ...
    for (i = 0; i < n; i++)
        tour[i] = original_tour[tour[i]];

    if (tour[0] != 0 || tour[1] == instance.n - 1)
        rotate_tour(tour, n);
    tour[n] = tour[0];
        ...
        for (l = 0; l < n; l++)
        {
            ...
            for (h = 0; h < local_nn_ls; h++)
            {
                ...
                if (radius > distance[c1][c2])
                {
                    ...
                    if (pos_c2 > 0)
                        ...
                        p_c2 = tour[n - 1];
                    gain = -radius + distance[c1][c2] + distance[c2][s_c1] + distance[p_c2][s_c2] - distance[c2][s_c2] - distance[p_c2][c2];
                    if (c2 == s_c1)
                        gain = 0;
                    if (p_c2 == s_c1)
                        gain = 0;
            ...
            for (h = 0; h < local_nn_ls; h++)
            {
                ...
                if (radius > distance[c1][c2])
                {
                    ...
                    s_c2 = tour[pos[c2] + 1];
                    gain = -radius + distance[c2][c1] + distance[p_c1][c2] + distance[p_c2][s_c2] - distance[c2][s_c2] - distance[p_c2][c2];
                    if (p_c1 == c2)
                        gain = 0;
                    if (p_c1 == s_c2)
                        gain = 0;
                ...
                while (g < local_nn_ls)
                {
                    ...
                    else
                    {
                        ...
                        if (decrease_breaks + add1 < add2)
                        {
                            ...
                            { /* not between(pos_c1,pos_c2,pos_c3) */
                                ...
                                gain = -radius - distance[p_c2][c2] -
                                       distance[c3][s_c3] + add1 + add2 + distance[p_c2][s_c3];
